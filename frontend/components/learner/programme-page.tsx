...
export function LearnerProgrammePage({ slug }: { slug: string }) {
  ...
  const enrollCourse = async (course: CourseCatalogItem) => {
    if (course.enrolled || enrolledCourseIds.includes(course.id) || enrollingCourseId) return;

    setEnrollingCourseId(course.id);
    try {
      await courseService.enroll(course.id);
      setEnrolledCourseIds((current) => current.includes(course.id) ? current : [...current, course.id]);
      toast.success("Enrollment complete — course access activated.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "We could not complete enrollment. Please try again.");
    } finally {
  ...
  const enrolledCount = courses.filter((course) => course.enrolled || enrolledCourseIds.includes(course.id)).length;
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {courses.map((course) => {
            ...
            const isEnrolled = course.enrolled || enrolledCourseIds.includes(course.id);
            ...
            return (
              <Card key={course.id} className="group flex h-full flex-col overflow-hidden rounded-xl border-[#f47945]/25 bg-white shadow-[0_10px_25px_rgba(77,23,110,0.05)] transition duration-300 hover:-translate-y-1 hover:border-[#f47945] hover:shadow-[0_16px_36px_rgba(77,23,110,0.1)]">
                ...
                <CardContent className="flex h-full flex-col gap-4 p-4 sm:p-5">
                  <div className="flex items-start justify-between gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-md text-xs font-black" style={{ backgroundColor: `${accent}18`, color: accent }}>{String(course.sequence).padStart(2, "0")}</span>{isEnrolled ? <span className="rounded-md bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700">In progress</span> : null}</div>
                  ...
                  {isEnrolled ? <div><div className="mb-1.5 flex items-center justify-between text-[11px] text-[#6e6072]"><span>{progress}% complete</span><span>{progress >= 100 ? "Complete" : "Keep going"}</span></div><div className="h-2 overflow-hidden rounded-sm bg-[#f2eaf4]"><div className="h-full rounded-sm transition-all" style={{ width: `${progress}%`, backgroundColor: accent }} /></div></div> : null}
                  <div className="mt-auto flex flex-col gap-2 pt-1"><Button asChild className="w-full rounded-md bg-[#4d176e] hover:bg-[#351039]"><Link href={`/learner/courses/${course.id}`}>{isEnrolled ? (progress > 0 ? "Continue course" : "Start course") : "View course"}<ArrowRight className="ml-1.5 h-4 w-4" aria-hidden /></Link></Button>{!isEnrolled ? <Button className="w-full rounded-md bg-[#f47945] text-[#351039] hover:bg-[#ff8f57]" onClick={() => enrollCourse(course)} disabled={enrollingCourseId === course.id}>{enrollingCourseId === course.id ? "Enrolling…" : "Enrol now"}</Button> : null}</div>
                </CardContent>
              </Card>
            );
          })}
