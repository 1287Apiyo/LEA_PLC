...
import { CalendarCheck2, CheckCircle2, Clock3, MapPin, MessageSquare, UserRound } from "lucide-react";
...
function RequestCard({ request }: { request: TutorRequest }) {
  ...
  const [venue, setVenue] = useState(request.venue ?? (request.mode === "in_person" ? "Applewood Adams, 13th Floor" : "Online meeting link to follow"));
    ...
    <Card className="border-border/80">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="space-y-1.5 text-sm"><span className="font-medium">Venue or meeting link{requiresSchedule ? " *" : ""}</span><div className="relative"><MapPin className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={venue} onChange={(event) => setVenue(event.target.value)} placeholder="Applewood Adams or meeting link" /></div></label>
          {requiresSchedule ? <>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button onClick={() => mutation.mutate()} disabled={mutation.isPending || (requiresSchedule && (!confirmedDate || !confirmedTime || !venue))}>{mutation.isPending ? "Saving response…" : "Save response"}</Button>
