...
function isScratchProject(row: ResourceRow) {
  return row.kind === "scratch_project" || row.editor === "scratch";
}

function isCodingProject(row: ResourceRow) {
  return row.kind === "coding_project" || row.kind === "workspace_snippet" || row.editor === "coding" || (typeof row.code === "string" && Boolean(row.language));
}

function kindLabel(row: ResourceRow) {
  if (isScratchProject(row)) return "Scratch project";
  if (row.language) return `${String(row.language).toUpperCase()} project`;
  return isCodingProject(row) ? "Coding project" : "Saved project";
}

function technologyLabel(row: ResourceRow) {
  if (isScratchProject(row)) return "Blocks + animation";
...
function projectPreviewAsset(row: ResourceRow): ProjectArt {
  ...
  if (isScratchProject(row)) return "/lesson-art/block_explorer_hero.png";
...
function resumeHref(row: ResourceRow) {
  ...
  if (isScratchProject(row)) return `/learner/playground?mode=scratch&projectId=${id}`;
...
export function LearnerProjects() {
  ...
  const projects = useMemo(
    () => ((projectsQuery.data?.data ?? []) as ResourceRow[]).filter((project) => isScratchProject(project) || isCodingProject(project)),
