...
function recommendation(answers: Answers) {
  ...
  if (answers.goal === "foundations" || answers.experience === "new") {
    return {
      programme: "Basic Computer Knowledge",
      programmeId: "prg-dl",
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <Card className="overflow-hidden border-[#4d176e] bg-[#1f0d2e] text-white">
        <CardContent className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
          <div>
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-[#f6b39a]"><Compass className="h-4 w-4" aria-hidden /> Learning setup</div>
            <h1 className="mt-3 max-w-2xl text-3xl font-semibold tracking-[-0.045em] sm:text-4xl">Let’s find the right place to begin.</h1>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-white/70">Answer four short questions. Your responses help LEA recommend a practical starting programme and the kind of support that will help you keep moving.</p>
          </div>
          <div className="rounded-2xl border border-white/15 bg-white/10 p-4"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#ffd3bd]">Your recommendation</p><p className="mt-2 text-xl font-semibold">{result.programme}</p><p className="mt-1 text-sm leading-6 text-white/70">{result.tone}</p></div>
        </CardContent>
      </Card>
